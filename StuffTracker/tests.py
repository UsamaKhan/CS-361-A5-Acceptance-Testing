...
class LoginList(TestCase):
    ...
    def test_correctName(self):
        for i in self.thingList.keys():
            ...
            self.assertEqual(self.monkey.session["name"], i, "name not passed from login to session")
    ...
    def test_precise(self):
        # complete, makes sure that there are no extra items in any owner's page
        for i in self.thingList.keys():
            resp = self.monkey.post("/", {"name": i, "password": i}, follow=True)
...
class LoginFail(TestCase):
    ...
    def test_noPassword(self):
        for i in self.thingList.keys():
            ...
            self.assertIn("bad password", resp.context["message"], "incorrect password message not displayed")
...
class AddItem(TestCase):
    # need to create database in setup confirm that after an add item form is submitted, that the new item is in the
    # database and appears in the response webpage
    monkey = None
    thingList = None
